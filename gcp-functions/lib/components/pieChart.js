 ...
 */
function createPieSlice(percentage, startAngle) {
    const radius = 61;
    const centerX = 61;
    const centerY = 61;
    // Convert to radians
...
export function generatePieChart(data) {
    ...
    const legendItems = data.segments
        ...
        const legendColor = segment.color || LEGEND_COLORS[index % LEGEND_COLORS.length];
      ...
      >
        ...
        <p style="flex-grow: 0; flex-shrink: 0; font-size: 14px; text-align: left; color: #333643;">
          <span style="flex-grow: 0; flex-shrink: 0; font-size: 14px; text-align: left; color: #333643;">${segment.label} </span>
