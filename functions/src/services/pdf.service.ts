...
export class PDFService {
  ...
  async generateTestSingleDomainPDF(options: PDFOptions = {}): Promise<Buffer> {
    try {
      ...
      // Generate both PDFs in parallel
      const [coverPdf, mainPdf] = await Promise.all([
        this.generateSingleDomainCoverPDF(Cover, mockSingleDomainData, options),
...
<body>${Cover({ timeperiod: data.timeperiod || 'Jul 30, 2025 - Aug 30, 2025', dateIssued })}</body>
  ...
  /**
   * Generate cover page PDF for Single Domain without header/footer (full-bleed design)
   */
  private async generateSingleDomainCoverPDF(
    Cover: (props: { timeperiod: string; dateIssued?: string }) => string,
    data: any,
    options: PDFOptions
  ): Promise<Buffer> {
...
<body>${Cover({ timeperiod: data.timePeriod || 'Jul 30, 2025 - Aug 30, 2025', dateIssued })}</body>
  ...
  private async generateSingleDomainMainContentPDF(
    ...
    return this.generateSingleDomainCleanPDFFromHTML(html, data, options);
  }

  /**
   * Merge multiple PDFs into one
   */
  private async mergePDFs(coverPdf: Buffer, mainPdf: Buffer, title: string = 'Partnership Domains Report'): Promise<Buffer> {
  ...
  /**
   * Clean PDF generation with Puppeteer's built-in header/footer
   * No custom styling or post-processing
   */
  private async generateCleanPDFFromHTML(
    html: string,
    options: PDFOptions
  ): Promise<Buffer> {
    if (!this.browser) {
    ...
    try {
      ...
      // Generate PDF with Puppeteer's default header/footer and margins
      const pdfBuffer = await page.pdf({
        format: options.format || 'A4',
        printBackground: true,
        preferCSSPageSize: false, // Use Puppeteer's settings, not CSS
        displayHeaderFooter: true, // Enable built-in header/footer
        headerTemplate: `
          <style>
          ...
          <div style="position: absolute; top: 0; left: 0; z-index: 999; width: 100%; padding: 24px 40px; background-color: #ffffff !important; -webkit-print-color-adjust: exact; font-family: 'DM Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
            ...
            >
              ...
              >
                <p
                  style="flex-grow: 1; width: 100%; font-size: 20px; font-weight: 700; text-align: left; color: #333643 !important; margin: 0;"
                >
              ...
              >
                ...
                >
                  <div
                    style="display: flex; flex-wrap: wrap; justify-content: flex-start; align-items: center; flex-grow: 1; gap: 6px; padding-top: 8px; padding-bottom: 8px; background-color: #ffffff !important; -webkit-print-color-adjust: exact;"
                  >
                    <div
                      style="display: flex; justify-content: flex-start; align-items: center; flex-grow: 0; flex-shrink: 0; overflow: hidden; padding-left: 4px; padding-right: 4px; padding-top: 2px; padding-bottom: 2px; border-radius: 100px; background-color: #f5f5f8 !important; -webkit-print-color-adjust: exact;"
                    >
                      <div
                      ...
                      >
                        <p
                          style="flex-grow: 0; flex-shrink: 0; font-size: 14px; text-align: left; color: rgba(0,0,0,0.87) !important; margin: 0;"
                        >
        ...
        `,
        margin: {
          top: '148px',   // Space for header
          right: '40px',
          bottom: '64px', // Space for footer (reduced - footer sticks to bottom)
          left: '40px',
        },
      });
  ...
  /**
   * Clean PDF generation for Single Domain with Puppeteer's built-in header/footer
   */
  private async generateSingleDomainCleanPDFFromHTML(
    html: string,
    data: any,
    options: PDFOptions
  ): Promise<Buffer> {
    ...
    try {
      ...
      const pdfBuffer = await page.pdf({
            ...
            >
              ...
              >
                <p
                  style="flex-grow: 1; width: 100%; font-size: 20px; font-weight: 700; text-align: left; color: #333643 !important; margin: 0;"
                >
                  ${data.domainName || 'Single Domain'} Influence Report
                </p>
                <span style="font-size: 10px; color: #666666 !important; white-space: nowrap;">
                  Page <span class="pageNumber"></span> of <span class="totalPages"></span>
                </span>
                    ...
                    >
                      ...
                      >
                        <p
                          style="flex-grow: 0; flex-shrink: 0; font-size: 14px; text-align: left; color: rgba(0,0,0,0.87) !important; margin: 0;"
                        >
                          Time period: ${data.timePeriod || 'Jul 30, 2025 - Aug 30, 2025'}
                        </p>
                      </div>
                    </div>
                    <div
        ...
        `,
        margin: {
          top: '132px',   // Slightly smaller header for single domain
          right: '40px',
          bottom: '64px',
          left: '40px',
        },
